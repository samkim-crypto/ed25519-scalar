...
const toU8 = (a, len) => au8(str(a) ? etc.hexToBytes(a) : u8n(au8(a)), len); // norm(hex/u8a) to u8a
const b2h = (b) => Array.from(b).map(e => padh(e, 2)).join(''); // bytes to hex
const n2b_32LE = (num) => etc.hexToBytes(padh(num, 32 * 2)).reverse(); // number to bytes LE
const b2n_LE = (b) => BigInt('0x' + b2h(u8n(au8(b)).reverse())); // bytes LE to num
const modL_LE = (hash) => etc.mod(b2n_LE(hash), N); // modulo L; but little-endian
...
export { getPublicKey, getPublicKeyAsync, sign, signAsync, sha };
